...
const chsubmit = function (val,idd) {
    ...
    // console.log(val);
    //+result._id
    axios.post('http://'+localStorage.getItem("backip")+'/user/update/'+idd, val)
        .then(
...
class Edituseradmin extends React.Component {
    ...
    async componentDidMount(){
        console.log("3")
        let ur = "http://"+localStorage.getItem("backip")+"/user/" + this.props.userid;
        await axios.get(ur).then(res => {
            ...
            fname=this.state.userde.us[0].firstName;
            lname=this.state.userde.us[0].lastName;
            mail=this.state.userde.us[0].email;
            add= this.state.userde.us[0].address;
            pno2= this.state.userde.us[0].pno;
            k = this.state.userde.us[0].types.value;
            se = se();
            console.log("1")
